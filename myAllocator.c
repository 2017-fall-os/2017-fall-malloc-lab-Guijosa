...
/* lowest & highest address in arena, and the last allocated prefix for nextfit (global vars) */
...
void freeRegion(void *r) {
	...
	if(p == lastAllocated){
	  lastAllocated = getPrevPrefix(p); /* to have a lastAllocated */
	}
	coalesce(p);
    }
}
